...
async function askGPT(userQuestion) {
  try {
    // Create message
    const message = await openai.beta.threads.messages.create("thread_ODMifwdrmOOOluvNm14O2PPN", {
      role: "user",
      content: userQuestion,
    });

    // Run assistant
    const run = await openai.beta.threads.runs.create("thread_ODMifwdrmOOOluvNm14O2PPN", {
    ...
    // Retrieve run log
    const runs = await openai.beta.threads.runs.retrieve("thread_ODMifwdrmOOOluvNm14O2PPN", "run_nFXVi6P02g4JJrnKr9Sp57ZS");

    // List messages
    const messages = await openai.beta.threads.messages.list("thread_ODMifwdrmOOOluvNm14O2PPN");
